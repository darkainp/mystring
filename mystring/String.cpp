...
****************************************************************/
String String::projection(char x)
{
	String str(x, 1);	//Convert char x to a string with size 1
...
* @param ch: a constant character.								*
****************************************************************/
String String::assign(const String& ch, int n)
{
	//Delete and reset
	delete[] value;
	value = nullptr;
	...
	if (n < 0 || n >= ch.length())
	{
		...
		if (n >= ch.length())
			cout << n << " exceed the length of the String." << endl;
		exit(EXIT_FAILURE);
	}
